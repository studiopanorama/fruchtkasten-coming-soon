...
function animateShift(totalShiftX, duration = 300) {
  const steps = 30;
  const shiftPerFrame = totalShiftX / steps;
  let current = 0;
  ...
  function step() {
    ...
    //   Matter.Body.translate(body, { x: shiftPerFrame, y: 0 });
    // });

    const effectiveWidth = getEffectiveWidth();
activeShapes.forEach(body => {
  if (body.position.x > effectiveWidth) {
    Matter.Body.translate(body, { x: shiftAmount, y: 0 });
  }
});

    // Rechte Wand bewegen
    const wallShift = shiftPerFrame;
    Matter.Body.translate(rightWall, { x: wallShift, y: 0 });
    const vertices = rightWall.vertices.map(v => ({ x: v.x + wallShift, y: v.y }));
    Matter.Body.setVertices(rightWall, vertices);
...
function updateRightWall() {
  ...
  Matter.Body.setVertices(rightWall, [
    ...
    { x: effectiveWidth + wallThickness, y: 0 },
    { x: effectiveWidth + wallThickness, y: window.innerHeight * 2 },
    { x: effectiveWidth, y: window.innerHeight * 2 }
  ]);

  // Canvas optisch verkleinern (nur Darstellung, nicht Physics)
...
animateShift(shift);
    const shiftAmount = -sidebar.offsetWidth;
activeShapes.forEach(body => {
  Matter.Body.translate(body, { x: shiftAmount, y: 0 });
});
    setTimeout(() => {
...
setTimeout(() => {
  // Jetzt ist Sidebar sichtbar geschlossen
  const effectiveWidth = getEffectiveWidth();
  activeShapes.forEach(body => {
    if (body.position.x > effectiveWidth) {
  ...
  animateWallShift(shift); // wenn du die Wand auch verschiebst
}, 310); // etwas länger als Sidebar-Animation (300ms)
    const shiftAmount = sidebar.offsetWidth;
activeShapes.forEach(body => {
  Matter.Body.translate(body, { x: shiftAmount, y: 0 });
});
...
if (isMobile) {
  ...
  maxShapes = 15; // kleines Notebook oder Tablet im Querformat
} else if (window.innerWidth < 1440) {
  maxShapes = 20; // normaler Laptop oder Desktop
} else if (window.innerWidth < 1800) {
  maxShapes = 25; // normaler Laptop oder Desktop
} else {
